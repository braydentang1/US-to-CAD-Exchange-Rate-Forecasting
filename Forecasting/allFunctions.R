...
innerTrain = function(trainFoldIndex, data, iterations, xreg){
  ...
  randomGrid = tibble(ETS.Weight = sample.int(10000, size = iterations, replace = TRUE), Theta.Weight = sample.int(10000, size = iterations, replace = TRUE),
                      ...
                      STL.Weight = sample.int(10000, size = iterations, replace = TRUE), ARIMA.Weight = sample.int(10000, size = iterations, replace = TRUE)) %>% 
  ...
  allInnerSets = createTimeSlices(y = outerTrain, initialWindow = 170, horizon = 3, fixedWindow = FALSE)
  ...
  for(k in 1:length(allInnerTrain)){
    ...
    allPredictions = grabPredictions(train = train, newdata = validation, xreg.train = xreg.train, xreg.newdata = xreg.validation) %>% 
      reduce(ts.intersect) %>% 
      as_tibble(.) %>%
      set_names(c("ETS", "Theta", "RWD", "TBATS", "STL", "ARIMA"))
...
grabPredictions = function(train, newdata, xreg.train, xreg.newdata){
  ...
  model.Average[[6]] = forecast.arima$mean
...
randomGridSearch = function(grid, predictions, validation){
  ...
  for (k in 1:nrow(grid)){
    ...
    predictions.tmp = predictions %>% transmute(Weighted.Average = ETS * as.numeric(grid[k,1]/grid[k, constant]) + Theta * as.numeric(grid[k,2]/grid[k, constant]) +
                                                  RWD * as.numeric(grid[k,3]/grid[k, constant]) + TBATS * as.numeric(grid[k,4]/grid[k, constant]) + STL * as.numeric(grid[k,5]/grid[k,constant])
                                                + ARIMA * as.numeric(grid[k,6]/grid[k, constant]))
